...
/// The camera's view-projection: world space straight to Vulkan clip space.
pub fn viewProjection(self: Self, aspect: f32) Mat4 {
...
pub fn move(self: *Self, dx: f32, dy: f32, dz: f32) void {
...
pub fn look(self: *Self, dyaw: f32, dpitch: f32) void {
    self.yaw += dyaw;
...
test "pitch is clamped" {
    ...
    try std.testing.expect(cam.pitch <= Self.max_pitch);
}
